...
try {
    ...
    if ($payment['status'] === 'captured') {
        $sql = "UPDATE rudhraabhishek_bookings 
                SET razorpay_payment_id = ?, payment_status = 'paid', booking_status = 'confirmed'
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode([
                'status' => 'info',
                'message' => 'Payment verified but booking was already updated.',
            ]);
        }
    } else {
        $sql = "UPDATE rudhraabhishek_bookings 
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Payment was not captured. Please contact support.',
        ]);
    }
} catch (Exception $e) {
